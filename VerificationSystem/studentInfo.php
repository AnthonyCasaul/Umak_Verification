...
if(mysqli_num_rows($select) > 0){
			while($row = mysqli_fetch_assoc($select)){
                ...
                $address = ucwords(strtolower($row['student_address']));
                ...
                $degree = ucwords(strtolower($row['degree']));
                $major = ucwords(strtolower($row['major']));
                $semester = $row['semester'];
                $acadyear = $row['academic_year'];
                $gname = ucwords(strtolower($row['guardian_name']));
                $gcontact = $row['guardian_contact'];
                $relationship = ucwords(strtolower($row['guardian_relationship']));
              }
}
?>
<!DOCTYPE html>
  ...
  <body>
    ...
    <div class="container">
      ...
      <table>
        ...
        <!-- 6TH ROW INPUT -->
        <tr>
          <td><b>&nbsp&nbsp<?php echo $gcontact;?></b></td>
      ...
      ?>
        <form id="GFG" method="post" action="editStudentInfo.php">
